...
Updates use five generic operations instead of a scenario per field or field
combination:
...
keys are excluded from weight changes. The previous scenario names remain
...
Legacy scenario aliases remain supported for existing configurations, including
`UPDATE_SINGLE_FIELD`,
...
`POST_MATCH_WEIGHT_LIMIT_EXCEEDED`. New configurations should use the generic
`operation` block described above. A per-entity `updates` block can specify
