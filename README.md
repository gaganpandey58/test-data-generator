...
omitted, every Payment is `MATCHED`. Scenario counts must add up to the
Payment `count`:
...
```json
{
  ...
  "payments": {
    "professional": {
      "count": 5,
      "source_claims": "./existing-claims-professional.jsonl",
      "scenarios": {
        "MATCHED": 2,
        "REVERSAL": 1,
        "REPLACEMENT": 1,
...
Supported source scenarios are `MATCHED`, `REVERSAL`, `REPLACEMENT`,
`STALE`, and `ORPHAN`. `REPLACEMENT` selects only source Claims whose
...
only. The source Claim JSONL is never modified and no Claim output is
generated by this mode. Both Payment P and Payment I use the same mechanism;
their declared layouts and schemas determine which fields are emitted.
