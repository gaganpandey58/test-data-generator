"""Derive immutable Payment records from existing Claim JSONL input."""
...
def derive_payment_from_claim(
    ...
    _set_payment_defaults(payment, claim, profile)
...
def _set_payment_defaults(
    payment: dict[str, object], claim: Mapping[str, object], profile: str
) -> None:
    """Fill only declared Payment fields that cannot be copied from the Claim."""
    ...
    if profile == "payment-institutional":
        _set_institutional_amount_defaults(payment)


def _set_institutional_amount_defaults(payment: dict[str, object]) -> None:
    """Derive missing institutional payment amounts from Claim amounts."""
...
def _make_orphan(payment: dict[str, object], profile: str, seed: int, index: int) -> None:
    """Break one declared matching key while preserving its JSON type."""
...
def _number(value: object) -> int | float:
    """Return numeric source values as safe arithmetic operands."""
